...
This module defines symbolic names for the integer error codes
returned by the Callable Library.  The names to which the
error codes are assigned are the same names used in the 
Callable Library, all of which begin with CPXERR.  The error codes
are accessible as the third element of the args attribute of the
exception that is raised.  These symbolic names should be used to test
if a particular error has occurred.  For example, to test if a read
operation failed because of insufficient memory, your program should
use the statements:
...
CPXERR_IIS_SUB_ABORT = 1714
CPXERR_IIS_SUB_DETTIME_LIM = 1715
