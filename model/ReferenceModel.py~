...
model.Demand = Param(model.Overtime, within = NonNegativeReals, default = 0.0)
...
def demand_constraint_rule1(m,i):
    ...
    return constraint_expr >= m.Demand['SatOT']
...
def demand_constraint_rule2(m,i):
    ...
    return constraint_expr >= m.Demand['SunOT']
...
def demand_constraint_rule3(m,i):
    ...
    return constraint_expr >= m.Demand['MonOT']
...
def second_stage_cost_rule(m):
    ...
    for i in m.Scenarios:
        for j in m.Overtime:
            obj_expr += 5.556*m.OTScenario[i,j]
